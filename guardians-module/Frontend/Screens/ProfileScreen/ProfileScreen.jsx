...
export default function ProfileScreen({ navigation }) {
  const { patient, guardian, otherGuardians, logout } = useApp();
  ...
  const handleSignOut = () => {
    Alert.alert('Sign Out', 'Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          await logout();
    navigation.replace('GuardianWelcomeScreen');
        },
      },
    ]);
  };

  // Safety check — agar data load na hua ho
  if (!patient || !guardian) {
    return (
      <View style={styles.container}>
        ...
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          <Text style={{ color: '#888', fontSize: 14 }}>Loading profile...</Text>
        </View>
      </View>
    );
  }
        ...
        <View style={styles.card}>
          <View style={styles.profileRow}>
            <View style={[styles.avatar, { backgroundColor: '#dce8f8' }]}>
              <Text style={[styles.avatarText, { color: '#2979ff' }]}>
                {patient.initials || '--'}
              </Text>
            </View>
            <View>
          ...
          {[
            ...
            { label: 'Phone',            value: patient.phone, isLink: true },
            { label: 'Address',          value: patient.address },
            {
              label: 'Monitoring Since',
              value: patient.monitoringSince
                ? new Date(patient.monitoringSince).toLocaleDateString('en-GB', {
            ...
            <View
              key={i}
              style={[styles.infoRow, i === arr.length - 1 && { borderBottomWidth: 0 }]}
            >
        ...
        <View style={styles.card}>
          <View style={styles.profileRow}>
            <View style={[styles.avatar, { backgroundColor: '#d0f0e0' }]}>
              <Text style={[styles.avatarText, { color: '#1a7a50' }]}>
                {guardian.initials || '--'}
              </Text>
            </View>
            <View>
              ...
              <Text style={styles.profileSub}>
                {guardian.relation || 'Guardian'} • Active guardian
              </Text>
            </View>
          </View>
        ...
        <View style={styles.card}>
          {!otherGuardians || otherGuardians.length === 0 ? (
            <View style={styles.emptyRow}>
              <Text style={styles.emptyText}>No other guardians added</Text>
            </View>
          ) : (
            otherGuardians.map((g, i) => (
              ...
              >
                <View style={[styles.smallAvatar, { backgroundColor: g.color || '#fff3e0' }]}>
                  <Text style={[styles.smallAvatarText, { color: g.textColor || '#e67e22' }]}>
                    {g.initials || '--'}
                  </Text>
                </View>
                <View>
...
const styles = StyleSheet.create({
  ...
  profileRow:   { flexDirection: 'row', alignItems: 'center', gap: 14, paddingVertical: 14 },
