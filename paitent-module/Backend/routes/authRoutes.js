...
router.put("/vitals", authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      // Temperature °F to °C convert karo
      let tempCelsius = 37.0;
      if (temperature) {
        tempCelsius = parseFloat(((temperature - 32) * 5 / 9).toFixed(1));
      }

      // Latest watch reading se HR aur SpO2 lo (agar available ho)
      ...
      const newReading = new VitalReading({
        userId:      req.userId,
        timestamp:   now,
        heartRate:   lastReading?.heartRate   || 75,  // last value ya default
        bloodOxygen: lastReading?.bloodOxygen || 97,  // last value ya default
      ...
      console.log('✅ Manual vitals also saved to VitalReading for guardian');
    } catch (e) {
