  ...
  try {
    ...
    try {
      ...
      if (hrData.records.length > 0) {
        ...
        // Resting HR — minimum HR of the day
        let minHR = Infinity;
    ...
    try {
      ...
      if (spo2Data.records.length > 0) {
        ...
        // SpO2 drop — previous reading se difference
    ...
    try {
      ...
      // 7 din ke steps — har din alag alag
      const dailySteps = [];
      for (let i = 6; i >= 0; i--) {
    ...
    // ── Profile se height/weight (meters/kg) ─────────────
    ...
    // ── Payload aur endpoint decide karo ─────────────────
    let endpoint = '';
    let payload  = {};
