...
export default function SignUp() {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // ✅ FIX: Signup successful hone pe purana user ka sara data clear karo
        // Taake naye user ko SignIn pe purana data na dikhe
        ...
        console.log('🧹 Old user data cleared after signup');
    ...
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      ...
      <Animated.View style={[styles.card, { transform: [{ translateY: cardSlide }] }]}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <Animated.View style={[styles.inputContainer, { transform: [{ translateX: input1Slide }], opacity: input1Slide.interpolate({ inputRange: [0, 100], outputRange: [1, 0] }) }]}>
            <View style={styles.iconWrapper}>
              <Ionicons name="person-outline" size={20} color="#667eea" />
            </View>
            <TextInput style={styles.input} placeholder="Full Name" placeholderTextColor="#999" value={name} onChangeText={setName} autoCapitalize="words" editable={!loading} />
          </Animated.View>

          <Animated.View style={[styles.inputContainer, { transform: [{ translateX: input2Slide }], opacity: input2Slide.interpolate({ inputRange: [0, 100], outputRange: [1, 0] }) }]}>
            <View style={styles.iconWrapper}>
              <Ionicons name="mail-outline" size={20} color="#667eea" />
            </View>
            <TextInput style={styles.input} placeholder="Email" placeholderTextColor="#999" value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" editable={!loading} />
          </Animated.View>

          <Animated.View style={[styles.inputContainer, { transform: [{ translateX: input3Slide }], opacity: input3Slide.interpolate({ inputRange: [0, 100], outputRange: [1, 0] }) }]}>
            <View style={styles.iconWrapper}>
              <Ionicons name="lock-closed-outline" size={20} color="#667eea" />
            </View>
            <TextInput style={styles.input} placeholder="Password" placeholderTextColor="#999" value={password} onChangeText={setPassword} secureTextEntry={!showPassword} editable={!loading} />
          ...
          <Animated.View style={[styles.inputContainer, { transform: [{ translateX: input4Slide }], opacity: input4Slide.interpolate({ inputRange: [0, 100], outputRange: [1, 0] }) }]}>
            <View style={styles.iconWrapper}>
              <Ionicons name="lock-closed-outline" size={20} color="#667eea" />
            </View>
            <TextInput style={styles.input} placeholder="Confirm Password" placeholderTextColor="#999" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry={!showConfirmPassword} editable={!loading} />
          ...
          <Animated.View style={{ transform: [{ translateY: buttonSlide }, { scale: buttonScale }] }}>
            <TouchableOpacity style={[styles.signUpButton, loading && styles.signUpButtonDisabled]} onPress={handleSignUp} activeOpacity={0.9} disabled={loading}>
              {loading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.signUpButtonText}>Sign Up</Text>}
            </TouchableOpacity>
          </Animated.View>

          <Animated.View style={[styles.signInContainer, { transform: [{ translateY: signInSlide }], opacity: signInSlide.interpolate({ inputRange: [0, 50], outputRange: [1, 0] }) }]}>
