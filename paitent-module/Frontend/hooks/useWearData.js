...
export const useWearData = () => {
  ...
  // ✅ FIX: Current token track karo — user change detect karne ke liye
  const currentTokenRef = useRef(null);

  // ✅ FIX: Backend pe save karne se pehle token verify karo
  const saveToBackend = async ({
    ...
    try {
      ...
      // ✅ FIX: Token nahi hai — save mat karo
      if (!token) {
        console.log('⚠️ Token nahi mila, backend save skip');
        return;
      }

      // ✅ FIX: Agar token badal gaya (naya user login kiya) — save skip karo
      // Ye check karta hai ke purane user ka data naye user ke account mein na jaye
      if (currentTokenRef.current && currentTokenRef.current !== token) {
        console.log('⚠️ Token change detected — naya user hai, purana data save skip');
        currentTokenRef.current = token;
        return;
      }

      // ✅ Token store karo reference mein
      currentTokenRef.current = token;
  ...
  useEffect(() => {
    // ✅ FIX: App start pe current token store karo
  ...
  const fetchData = async () => {
    try {
      // ✅ FIX: Fetch se pehle token check karo
      const token = await AsyncStorage.getItem('userToken');
      if (!token) {
        console.log('⚠️ Token nahi hai — fetchData skip');
        return;
      }
      ...
      // ✅ Backend pe save karo — sirf tab jab HR ya SpO2 available ho
